...
**Development model.** Native Radiance first-hit checks match on 51,100 rays across ten cases; actual Chromium CPU/WebGPU checks agree on the documented rack and transmission fixtures. Independent pvlib solar and Radiance sky/daily comparisons, 66 convergence sweeps across both hemispheres and dense winter sampling studies are recorded in the [version 0.5.0 implementation report](docs/REVIEW-FIXES-2026-09-20.md). These checks do not establish measured field accuracy or independent PAR calibration. One low-sun Radiance sky case was rejected by the oracle; broad browser/device and upper-limit annual validation remain incomplete. Opaque monofacial modules or area-averaged bifacial cell-gap transmission; square posts and flat ground; no vegetation occlusion or reflected radiation. See [architecture](docs/ARCHITECTURE.md), [status](docs/STATUS.md), and the [original modeling brief](docs/modeling-brief.md).
