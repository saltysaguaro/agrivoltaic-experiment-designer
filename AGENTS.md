...
- Field layouts lock to receiver cells: sensors retain centre coordinates and independent installation heights/depths; crops occupy whole cell rectangles and rotate with the array. Pack display-only sensor dots inside their cells and use count badges for dense groups. Preserve grid indices across edits and clamp to bounds; normalize legacy/imported layouts. Receiver outlines must match actual cell boundaries in interactive and publication views.
...
- Address/place search uses the original public Mapbox key and geocoding endpoint through a new adapter, without archive runtime imports. Apply selected lat/lon atomically, refresh automatic weather, label longitude-derived UTC offsets as estimates, and retain manual coordinates. Full array defaults to oblique orthographic.
