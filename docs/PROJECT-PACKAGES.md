...
Pergolas store their fixed angle in `racking.pergolaTilt` (0–85°). Missing values default to 0°, retaining the horizontal geometry and valid results of older projects even if their formerly unused `racking.tilt` is nonzero. Both aligned and checkerboard layouts support this angle. Changing it requires recalculating light.
