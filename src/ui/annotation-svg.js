...
export function annotationSvg(
  ...
  annotations.map((a, i) => {
    ...
    if (a.kind === 'dimension') {
      ...
      if (length < 18) {
        // Edge-on and tiny dimensions need a point leader, not long coincident
        // witness lines or a made-up screen-space span.
        anchor = mid;
        projectionNote = ' Short or edge-on projection; the value is measured in 3D.';
        marks += segment(p, q, 'data-projected-edge="true"');
        marks += `<circle cx="${mid[0]}" cy="${mid[1]}" r="3"/>`;
      } else {
        normal = [-uy, ux];
        if ((mid[0] - center[0]) * normal[0] + (mid[1] - center[1]) * normal[1] < 0)
          normal = normal.map((n) => -n);
        const alternatives = [normal, normal.map((n) => -n)]
          .map((n, side) => {
            const clearance =
              a.external && obstacles.length
                ? Math.max(
                    0,
                    ...obstacles.map((v) => (v[0] - mid[0]) * n[0] + (v[1] - mid[1]) * n[1]),
                  )
                : 0;
            const requested = clearance + 18 + i * 8;
            let available = Infinity;
            for (const endpoint of [p, q])
              for (const axis of [0, 1]) {
                const low = axis === 0 ? 10 : top + 5,
                  high = axis === 0 ? width - 10 : height - 10;
                if (Math.abs(n[axis]) > 1e-8)
                  available = Math.min(
                    available,
                    ((n[axis] > 0 ? high : low) - endpoint[axis]) / n[axis],
                  );
              }
            const offset = Math.max(0, Math.min(requested, available));
            return {
              normal: n,
              offset,
              score: Math.max(0, requested - available) * 100 + requested + side * 8,
            };
          })
          .sort((a, b) => a.score - b.score);
        normal = alternatives[0].normal;
        const offset = alternatives[0].offset;
        ...
        marks +=
          segment(p, from, 'data-witness="true" opacity=".55"') +
          segment(q, to, 'data-witness="true" opacity=".55"') +
      ...
      const radius = Math.max(
        ...ps.slice(1).map((p) => Math.hypot(p[0] - ps[0][0], p[1] - ps[0][1])),
      );
      const area =
        Math.abs(
          ps.reduce((sum, p, i) => {
      ...
      if (area < Math.max(8, radius * radius * 0.015)) {
        anchor = ps[0];
        projectionNote =
          ' Angle plane is edge-on or the angle is zero; use Profile for tilt or Top-down for azimuth.';
        marks += `<circle cx="${anchor[0]}" cy="${anchor[1]}" r="3"/>`;
      } else {
    ...
    const distance = normal
      ...
      : 30;
    const natural = [
      anchor[0] + (normal ? normal[0] : 0) * distance - boxW / 2,
      anchor[1] + (normal ? normal[1] : -1) * distance - boxH / 2,
    ];
    ...
    if (obstacle)
      for (const shift of [-60, -30, 30, 60])
        candidates.push(
          [obstacle.left - boxW - 12, anchor[1] - boxH / 2 + shift],
          [obstacle.right + 12, anchor[1] - boxH / 2 + shift],
          [anchor[0] - boxW / 2 + shift, obstacle.top - boxH - 12],
          [anchor[0] - boxW / 2 + shift, obstacle.bottom + 12],
        );
    for (let row = top; row < height - boxH - 8; row += 30) {
    ...
    const scored = candidates
      .map(([x, y]) => [clamp(x, 8, width - boxW - 8), clamp(y, top, height - boxH - 8)])
      .map(([x, y]) => ({
        x,
        y,
        score:
          ...
          Math.hypot(x + boxW / 2 - anchor[0], y + boxH / 2 - anchor[1]) +
          Math.hypot(x - natural[0], y - natural[1]) * 0.15,
      }))
      .sort((a, b) => a.score - b.score);
    const { x, y } = scored[0];
    occupied.push({ x, y, w: boxW });
    const join = [clamp(anchor[0], x, x + boxW), clamp(anchor[1], y, y + boxH)];
    if (Math.hypot(join[0] - anchor[0], join[1] - anchor[1]) > 3)
      marks += segment(anchor, join, 'data-label-leader="true" opacity=".7"');
    drawings.push({
      a,
      color,
      marks,
      label: `<rect x="${x}" y="${y}" width="${boxW}" height="23" rx="3" fill="#ffffff" fill-opacity=".96"/><text x="${x + boxW / 2}" y="${y + 15.5}" text-anchor="middle" fill="${color}" stroke="none" font-size="12" font-family="Arial,sans-serif">${e(text)}</text>`,
      projectionNote,
    });
  });
  // Draw every label last, so another dimension's witness cannot cut its text.
  return (
    drawings
      .map(
        ({ a, color, marks }) =>
          `<g data-callout-marks="${e(a.id)}" fill="none" stroke="${color}" stroke-width="1.3">${marks}</g>`,
      )
      .join('') +
    drawings
      .map(
        ({ a, color, label, projectionNote }) =>
          `<g data-callout="${e(a.id)}" fill="none" stroke="${color}" stroke-width="1.3"><title>${e(a.label + ': ' + a.value + projectionNote)}</title>${label}</g>`,
      )
      .join('')
  );
}
