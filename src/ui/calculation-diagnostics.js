...
export function logCalculationDiagnostics(study, result, logger = console) {
  ...
  logger.info(
    `${result.backend} · ${study.analysis.patches} sky patches · ${study.analysis.interval}-minute direct steps · ${result.samplesPerCell ?? 1} samples per cell · reflection excluded`,
  );
